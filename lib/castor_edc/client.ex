defmodule CastorEDC.Client do
  ...
  Additionally it's possible to pass options to the underlying adapter (Hackney) e.g.
  ...
  def new(%{access_token: access_token}, endpoint, opts) when is_binary(endpoint) do
    validate_options!(opts)
    %__MODULE__{access_token: access_token, endpoint: endpoint(endpoint), options: opts}
  end

  defp validate_options!(opts) do
    supported_options = [:adapter]
